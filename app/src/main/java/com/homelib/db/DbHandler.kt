...
class DbHandler(context: Context): SQLiteOpenHelper(context,DATABASE_NAME,null,DATABASE_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        val CREATE_CONTACTS_TABLE = ("CREATE TABLE " + TABLE_BOOKS + "("
                + COL_ID + " INTEGER PRIMARY KEY," + COL_AUTHOR + " TEXT," + COL_IMAGE_LINK + " TEXT," + COL_YEAR + " TEXT,"
                + COL_TITLE + " TEXT" + ")")
    ...
    fun viewBooks():ArrayList<BookModel>{
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                val emp= BookModel(title = bookTitle, author = bookAuthor, year= bookYear,image = bookImageLink)
                empList.add(emp)
